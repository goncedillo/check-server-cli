...
async function checkServer(server) {
    ...
    try {
        ...
        const resultToPrint = statusResult ? emojis.unicode(':white_check_mark:') : emojis.unicode(':red_circle:');

        logger.log('info', `<${server.name.toUpperCase()}> ${dateToRegister} - ${resultToPrint}`);
        setWebStatus(server.name, now, statusResult)
    } catch (err) {
        logger.log('info', `<${server.name.toUpperCase()}> ${dateToRegister} - ${emojis.unicode(':red_circle:')}`);
